...
describe('subtitles API client', () => {
  ...
  it('uses shared subtitle, YouTube, and linguist routes with encoded queries', async () => {
    ...
    const youtubeLibraryUrl = new URL(String(fetchMock.mock.calls[4][0]));
    ...
    const streamsUrl = new URL(String(fetchMock.mock.calls[5][0]));
    ...
    expect(new URL(String(fetchMock.mock.calls[8][0])).pathname).toBe('/api/subtitles/jobs');
    expect(new URL(String(fetchMock.mock.calls[9][0])).pathname).toBe('/api/assistant/lookup');
  });
});
