...
import {
  ...
  formatImageSeconds,
  IMAGE_PROMPT_PIPELINE_OPTIONS,
  IMAGE_STYLE_OPTIONS,
  resolveImagePromptPipeline,
  resolveImageStyle,
  stepsFromQuality
} from './bookNarrationImageSettings';
...
const BookNarrationImageSection = ({
  ...
  const promptPipelineMeta = resolveImagePromptPipeline(imagePromptPipeline);
  const promptPipelineValue = promptPipelineMeta.id;
  const usesVisualCanon = promptPipelineValue === 'visual_canon';
  const style = resolveImageStyle(imageStyleTemplate);
  const { qualityValue, concurrencyValue, secondsPerImage, imagesPerMinute } = estimateImageGeneration({
    imageSteps,
    imageWidth,
    imageHeight,
    imageConcurrency,
      ...
      <div className="pipeline-card__body">
        ...
        {addImages ? (
          <>
            ...
            <p className="form-help-text">
              Estimate: ~{formatImageSeconds(secondsPerImage)} per image; with {concurrencyValue} worker
