...
- Apple deploy readiness now also has `make apple-device-deploy-readiness-dry-run`, registered as a credential-free shared-pipeline app-owned journey that chains host readiness, CoreDevice listing, and the default device dry-run matrix before any explicit device deploy.
...
- Advanced visible Apple app versioning to `v2026.07.04.006`.

### 2026.07.04.005

- Apple TV sequence playback now force-finishes a stuck reader-owned track handoff if tvOS never reports ready or seek completion, and keeps short sentence segments from being over-trimmed during same-track guards.
- Advanced visible Apple app versioning to `v2026.07.04.005`.

### 2026.07.04.004

- Apple YouTube Dub now uses prepared subtitle language hints from reviewed video discovery handoffs to set the target language when the user has not edited it, matching Web behavior before the NAS library row refreshes.
- Advanced visible Apple app versioning to `v2026.07.04.004`.

### 2026.07.04.003

- Apple TV sequence handoffs now have reader-owned recovery probes for initial loads, track switches, and same-track dwell seeks, so playback can re-seek and resume when tvOS misses a ready/seek completion instead of leaving the reader muted or stopped.
- Advanced visible Apple app versioning to `v2026.07.04.003`.

### 2026.07.04.002

- Apple TV sequence playback now recovers stale initial transitions when sentence audio is already requested and the AVPlayer is ready, preventing the reader from staying muted/transitioning until playback appears to stop.
- Advanced visible Apple app versioning to `v2026.07.04.002`.
...
- Apple Original-only and Translation-only playback now narrows combined audio options to the requested stream URL before loading, and tvOS sequence handoffs use a larger output-buffer guard to prevent a next-original-sentence sliver before Translation starts.
- Backend Library route access-policy helpers now live in a focused support module with direct coverage, keeping owner fallback, default-public visibility, and Web/Apple denial messages reusable outside the large router.
- Backend acquisition provider readiness resolution now lives in a focused helper while preserving the existing provider-registry compatibility hooks used by route/tests and Web/Apple Create discovery defaults.
...
- Backend acquisition prepared-artifact book/video source-root path validation now lives in a focused helper with direct coverage, keeping Web and Apple Create artifact handoffs shared outside the download orchestration path.
...
- Direct Apple interactive player embeds now mirror reader-owned Apple Music bed pauses into narration before reinforcing the Music pause, so reusable consumers without the Job/Library shell avoid split Music-only pauses.
...
- Apple Job and Library playback now refresh the Music-bed reader-active bridge from the requested-play state before Music surface changes and reader transport commands, so a single pause can combine Apple Music bed and sentence audio even during autoplay or track handoff timing gaps.
...
- Web and Apple sequence playback now trim overlapping same-track sentence gates just before the next sentence start while leaving clean adjacent gates intact, reducing the cut-short next-sentence sliver before Translation handoff.
- The Apple playback contract now pins the same-track handoff guard plus the Swift overlap/adjacent-gate harness, so the reusable pipeline catches regressions that would remove the output-buffer safety margin.
...
- The iPad music-bed simulator journey now publishes lookup-bubble counters at the front of its DEBUG status label, preventing accessibility truncation from hiding bubbleLookup evidence during unattended keyboard validation.
- The Apple TV music-bed simulator journey now retries its DEBUG observed-pause probe after interactive playback starts, keeping unattended validation from timing out when one delayed simulator callback is missed.
- Apple reader pause adoption now reclaims the Music bed when MusicKit receives the first pause edge, mirrors that adopted pause into Job and Library narration even if the system briefly owns Music, and keeps confirming both layers through the tvOS echo window.
- Apple reader pause handling now pauses sentence narration before the Apple Music bed, treats pause confirmation as incomplete while either layer is still active, and settles sequence-mode resume before restoring audio, so iPad and Apple TV no longer need separate presses for bed music and narration.
- Apple Create readiness now verifies acquisition provider display labels against the backend label catalog, catching raw provider ids before iPhone, iPad, Apple TV, or Mac iPad-style Create surfaces show them.
- Apple reader pause commands now latch and reinforce the Apple Music bed pause before pausing sentence narration, and pause confirmation retries reapply both sides so tvOS no longer needs a second press when MusicKit receives the first signal.
