...
- The backend acquisition route now treats a leaked `backend_defaults` provider id as the same no-provider Default sources fan-out used by Web and Apple Create, so older clients do not break book/video discovery.
