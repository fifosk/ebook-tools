...
def build_lookup_system_prompt(*, input_language: str, lookup_language: str) -> str:
    ...
    return "\n".join(
        [
            ...
            '    {"language": "Hindi", "word": "किताब", "transliteration": "kitāb"}',
            "  ]",
            "}",
            "",
            "Rules:",
            ...
            "- related_languages: For words/phrases, this is expected unless no reliable relation exists. Show up to 3 reliable cognates, ancestor/source forms, direct borrowings, or loanwords. Include transliteration for non-Latin scripts. null for sentences",
            "- This applies to every input language: first look for same-family cognates or root relatives, then ancestor/source forms, then documented borrowings or loanwords in neighboring/common contact languages",
            "- For Hebrew, Arabic, Aramaic, Syriac, Akkadian, and other Semitic words, actively look for Semitic cognates/root relatives before returning null",
            "- For loanwords, include the source language when known; otherwise include common cognates/borrowings in nearby languages",
            "- Avoid false cognates. If fewer than 3 reliable entries exist, return only the reliable ones",
            "- Do not omit the related_languages key. Use null only when you genuinely cannot identify a reliable related form",
            "",
