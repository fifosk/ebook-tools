...
from .library_metadata import LibraryMetadataError, LibraryMetadataManager
...
class LibrarySync:
    ...
    def refresh_metadata(self, job_id: str) -> LibraryEntry:
        ...
        with DirectoryLock(job_root):
            ...
            if epub_path is not None and epub_path.exists():
                try:
                    local_metadata = self._metadata_manager.infer_metadata_from_epub(
                        epub_path,
            ...
            if isbn_value:
                try:
                    isbn_metadata = self._metadata_manager.fetch_metadata_from_isbn(isbn_value)
    ...
    def lookup_isbn_metadata(self, isbn: str) -> Dict[str, Optional[str]]:
        ...
        if not normalized:
            raise LibraryError("ISBN must contain 10 or 13 digits (optionally including X)")
        return self._metadata_manager.fetch_metadata_from_isbn(normalized)
    ...
    def search(
        ...
        *,
        ...
        total = self._repository.count_entries(query=query, filters=_compact_filters(filters))
    ...
    def reindex_from_fs(self) -> int:
        ...
        items: List[LibraryEntry] = []
        state_dir = self._repository.db_path.parent
        ...
        self._repository.replace_entries(items)
        return len(items)
    ...
    def build_entry(self, metadata: Mapping[str, Any], job_root: Path) -> LibraryEntry:
        ...
        return self._build_item(dict(metadata), Path(job_root))
    ...
    def resolve_library_path(self, metadata: Mapping[str, Any], job_id: str) -> Path:
        ...
        return self._resolve_library_path(dict(metadata), job_id)
    ...
    def _merge_metadata_payloads(self, *payloads: Mapping[str, Any]) -> Dict[str, Any]:
        return self._metadata_manager.merge_metadata_payloads(*payloads)

    def _build_item(self, metadata: Dict[str, Any], job_root: Path) -> LibraryEntry:
        job_id = str(metadata.get("job_id") or "").strip()
        if not job_id:
