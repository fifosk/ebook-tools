...
def test_generate_video_slides_ffmpeg_retries_sentence_merge(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    text_blocks = ["Target - Translation"]
    audio_segments = [AudioSegment.silent(duration=1000)]
    ...
    def fake_export(self, out_f, format=None, bitrate=None, **kwargs):  # type: ignore[override] # noqa: ANN001, ARG002
        Path(out_f).write_bytes(b"audio")
        return out_f
    ...
    def fake_run(cmd, stdout=None, stderr=None):  # noqa: ANN001
        run_calls.append(cmd)
        if cmd[-1].endswith("merged.mp4") and len(run_calls) == 2:
            return subprocess.CompletedProcess(cmd, 183, stdout=b"", stderr=b"File exists")
        if cmd[-1].endswith("merged.mp4"):
            Path(cmd[-1]).write_bytes(b"merged-video")
        elif cmd[-1].endswith("final.mp4"):
            Path(cmd[-1]).write_bytes(b"final-video")
        return subprocess.CompletedProcess(cmd, 0, stdout=b"", stderr=b"")
    ...
    merge_invocations = [call for call in run_calls if call[-1].endswith("merged.mp4")]
    assert len(merge_invocations) == 2
    assert any("libx264" in call for call in merge_invocations)
    assert os.path.exists(final_path)


def test_generate_video_slides_ffmpeg_handles_pad_failure(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    text_blocks = ["Target - Translation"]
    audio_segments = [AudioSegment.silent(duration=1000)]
    ...
    def fake_run(cmd, stdout=None, stderr=None):  # noqa: ANN001
        if cmd[-1].endswith("merged.mp4"):
            Path(cmd[-1]).write_bytes(b"merged-video")
        if cmd[-1].endswith("final.mp4"):
            return subprocess.CompletedProcess(cmd, 254, stdout=b"", stderr=b"Pad failure")
    ...
    assert os.path.exists(final_path)
    assert Path(final_path).read_bytes() in {b"merged-video", b"ok"}
