...
def test_mode_switch_integration_check_is_wired_into_apple_contracts() -> None:
    ...
    assert "Original + Translation role intent should resolve to sequence mode even without a combined audio option" in swift_check
...
def test_transcript_display_snapshot_check_is_wired_into_apple_contracts() -> None:
    ...
    assert "preScaledV2Sentences" in swift_check
    assert "Timing v2 live highlighting should preserve backend-scaled translation reveal times during playback" in swift_check
...
def test_apple_music_manual_pause_blocks_auto_resume_during_sentence_switch() -> None:
    ...
    assert "shouldConfirmActiveNarrationNonPlayingAsReaderPause" not in observed_pause_body
