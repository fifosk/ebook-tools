...
def test_now_playing_remote_commands_cover_text_video_and_bookmarks() -> None:
    ...
    assert "@Published private(set) var isE2ERequestedTransitionPauseActive = false" in audio
    assert "isE2ERequestedTransitionPauseActive = true" in _function_body(
        audio,
        "func simulateRequestedTransitionPauseForMusicBedE2E()",
    )
    assert "isE2ERequestedTransitionPauseActive = false" in _function_body(
        audio,
    ...
    assert "private var shouldRejectUnsolicitedReaderPlayCommand: Bool" in job_now_playing
    assert "Job reader transport play command ignored unsolicited reader-pause echo" in job_now_playing
    ...
    assert "@State var readerTransportResumeGeneration = 0" in job_playback
    ...
    assert "TVPlayPauseCommandGate.shouldSuppressCurrentPress()" in job_playback
    assert "Job foreground tvOS Play/Pause ignored duplicate physical press" in job_playback
    ...
    assert "Job foreground tvOS Play/Pause ignored reader transport pause echo" not in job_playback
    assert "shouldForceTVReaderNowPlayingResume(ignorePauseHold: true)" in job_playback
    assert 'forcePlayReaderNowPlayingTransport(source: "brokerRequestedResume")' in job_playback
    assert 'forcePlayReaderNowPlayingTransport(source: "foregroundRequestedResume")' in job_playback
    ...
    assert "guard viewModel.audioCoordinator.isPlaybackRequested || viewModel.audioCoordinator.isPlaying else" not in job_adoption_body
    assert job_adoption_body.index("if shouldIgnoreRequestedAppleMusicPauseBeforeReaderAudible") < job_adoption_body.index(
    ...
    assert "if musicOwnership.ownershipState == .appleMusicBed" in job_mirror_pause_decision_body
    assert "!musicOwnership.isPlaying &&" not in job_mirror_pause_decision_body
    assert "!musicOwnership.isSystemPlaybackPlaying" not in job_mirror_pause_decision_body
    ...
    assert "hasPendingReaderMusicResume" in job_stale_pause_body
    ...
    assert "pendingInteractiveAutoplaySentence != nil" in job_stale_pause_body
    ...
    assert 'reason == "observedNonPlaying" || reason == "deferredObservedNonPlaying"' not in job_honor_adoption_body
    assert 'source == "active observed non-playing"' not in job_honor_adoption_body
    assert 'source == "persistent observed non-playing"' not in job_honor_adoption_body
    ...
    assert "ReaderTransportCommandResolver.shouldAcceptActiveReaderDuplicatePause" in job_accept_body
    assert "ReaderTransportCommandResolver.shouldRejectDuplicateCommand" in job_accept_body
    assert "isReaderPlaybackRequested: viewModel.audioCoordinator.isPlaybackRequested" in job_accept_body
    assert "isReaderPlaying: viewModel.audioCoordinator.isPlaying" in job_accept_body
    ...
    assert "command accepting duplicate pause for active reader" in job_accept_body
    ...
    assert "ReaderTransportCommandResolver.shouldReinforcePauseAfterRejectedPlay(" in job_reinforce_body
    assert "ownershipState: musicOwnership.ownershipState" in job_reinforce_body
    assert "resolvedAction: resolvedAction" in job_reinforce_body
    assert "previousAction: lastReaderTransportAction" in job_reinforce_body
    assert "now: now" in job_reinforce_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in job_reinforce_body
    assert "shouldRejectResumeAfterPause: musicOwnership.shouldRejectReaderTransportResumeAfterPause" in job_reinforce_body
    assert "isPauseHoldWindowActive: musicOwnership.isReaderTransportPauseHoldWindowActive" in job_reinforce_body
    assert "isPauseGuardActive: musicOwnership.isReaderTransportPauseGuardActive" in job_reinforce_body
    ...
    assert "ReaderTransportCommandResolver.shouldForceNowPlayingPause(" in job_force_pause_body
    assert "ownershipState: musicOwnership.ownershipState" in job_force_pause_body
    assert "shouldForceTVReaderNowPlayingResume()" in job_force_pause_body
    assert "previousAction: lastReaderTransportAction" in job_force_pause_body
    assert "now: ProcessInfo.processInfo.systemUptime" in job_force_pause_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in job_force_pause_body
    assert "shouldRejectResumeAfterPause: musicOwnership.shouldRejectReaderTransportResumeAfterPause" in job_force_pause_body
    assert "isPauseHoldWindowActive: musicOwnership.isReaderTransportPauseHoldWindowActive" in job_force_pause_body
    job_requested_resume_body = _function_body(job_now_playing, "func shouldRecoverTVReaderNowPlayingRequestedPlayback()")
    assert "musicOwnership.ownershipState == .appleMusicBed" in job_requested_resume_body
    assert "viewModel.audioCoordinator.isPlaybackRequested" in job_requested_resume_body
    assert "!viewModel.audioCoordinator.isPlaying" in job_requested_resume_body
    assert "!viewModel.isNarrationAudibleForReaderTransport" in job_requested_resume_body
    assert "musicOwnership.isPlaying" in job_requested_resume_body
    assert "!musicOwnership.isPausedByReaderTransport" in job_requested_resume_body
    assert "!musicOwnership.isReaderTransportPauseGuardActive" in job_requested_resume_body
    assert 'lastReaderTransportAction != "pause"' in job_requested_resume_body
    ...
    assert "ReaderTransportCommandResolver.shouldForceNowPlayingResume(" in job_force_resume_body
    assert "ownershipState: musicOwnership.ownershipState" in job_force_resume_body
    assert "previousAction: lastReaderTransportAction" in job_force_resume_body
    assert "ignorePauseHold: ignorePauseHold" in job_force_resume_body
    assert "now: ProcessInfo.processInfo.systemUptime" in job_force_resume_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in job_force_resume_body
    assert "isReaderPlaybackRequested: viewModel.audioCoordinator.isPlaybackRequested" in job_force_resume_body
    ...
    assert "shouldIgnoreTVReaderTransportBrokerEcho()" not in job_foreground_body
    assert "Job foreground tvOS Play/Pause ignored reader transport pause echo" not in job_foreground_body
    assert job_foreground_body.index("Job foreground tvOS Play/Pause command") < job_foreground_body.index(
    ...
    assert "ReaderTransportCommandResolver.shouldIgnoreBrokerEcho(" in job_broker_echo_body
    assert "canForceResume: shouldForceTVReaderNowPlayingResume(ignorePauseHold: true)" in job_broker_echo_body
    ...
    assert "static func shouldAcceptActiveReaderDuplicatePause" in transport_resolver
    assert "static func shouldRejectUnsolicitedPlayCommand" in transport_resolver
    assert "static func shouldIgnoreBrokerEcho" in transport_resolver
    assert "static func shouldBlockResumeAfterPause" in transport_resolver
    assert "static func shouldReinforcePauseAfterRejectedPlay" in transport_resolver
    assert "enum TVPlayPauseCommandGate" in transport_resolver
    assert "static func shouldSuppressCurrentPress()" in transport_resolver
    assert "static var hardwarePressEchoWindow: TimeInterval" in transport_resolver
    hardware_echo_body = _function_body(transport_resolver, "static var hardwarePressEchoWindow: TimeInterval")
    assert "return 0.05" in hardware_echo_body
    assert "suppressUntil = now + ReaderTransportCommandResolver.hardwarePressEchoWindow" in transport_resolver
    ...
    reapply_body = _function_body(
        transport_resolver,
        "static func shouldReapplyDuplicateCommand",
    )
    reject_body = _function_body(
        transport_resolver,
        "static func shouldRejectDuplicateCommand",
    )
    active_duplicate_pause_body = _function_body(
        transport_resolver,
    ...
    assert 'previousAction == "pause"' in active_duplicate_pause_body
    assert "(isReaderPlaybackRequested || isReaderPlaying)" in active_duplicate_pause_body
    assert "return false" in active_duplicate_pause_body
    unsolicited_play_resolver_body = _function_body(
        transport_resolver,
        "static func shouldRejectUnsolicitedPlayCommand",
    )
    force_pause_resolver_body = _function_body(
        transport_resolver,
        "static func shouldForceNowPlayingPause",
    )
    force_resume_resolver_body = _function_body(
        transport_resolver,
        "static func shouldForceNowPlayingResume",
    )
    broker_echo_resolver_body = _function_body(
        transport_resolver,
        "static func shouldIgnoreBrokerEcho",
    )
    block_resume_resolver_body = _function_body(
        transport_resolver,
        "static func shouldBlockResumeAfterPause",
    )
    reinforce_pause_resolver_body = _function_body(
        transport_resolver,
        "static func shouldReinforcePauseAfterRejectedPlay",
    )
    can_resume_body = _function_body(
        transport_resolver,
        "private static func canResumeAfterReaderPause",
    )
    assert "#if os(tvOS)" in reapply_body
    ...
    assert "shouldRejectResumeAfterPause" in unsolicited_play_resolver_body
    assert "isPauseHoldWindowActive" in unsolicited_play_resolver_body
    assert "now < localPauseHoldUntil" in unsolicited_play_resolver_body
    assert "isReaderTransportPauseGuardActive" not in unsolicited_play_resolver_body
    assert "ownershipState == .appleMusicBed" in force_pause_resolver_body
    assert "if isReaderPlaybackRequested || isReaderPlaying" in force_pause_resolver_body
    assert force_pause_resolver_body.index("if isReaderPlaybackRequested || isReaderPlaying") < force_pause_resolver_body.index(
        "return !canResumeAfterReaderPause("
    )
    assert "return !canResumeAfterReaderPause(" in force_pause_resolver_body
    assert "return isReaderPlaybackRequested || isReaderPlaying" in force_pause_resolver_body
    ...
    adopted_window_body = _function_body(transport_resolver, "static var adoptedMusicPauseBrokerEchoWindow: TimeInterval")
    ...
    assert 'guard resolvedAction == "play" else { return false }' in block_resume_resolver_body
    assert "now < localPauseHoldUntil" in block_resume_resolver_body
    assert "shouldRejectResumeAfterPause" in block_resume_resolver_body
    assert "isPauseHoldWindowActive" in block_resume_resolver_body
    assert "guard shouldHoldReaderResumeAfterPause else { return false }" in reinforce_pause_resolver_body
    assert 'guard resolvedAction == "play" else { return false }' in reinforce_pause_resolver_body
    assert "guard ownershipState == .appleMusicBed else { return false }" in reinforce_pause_resolver_body
    assert "let shouldAllowPostPauseResume = canResumeAfterReaderPause(" in reinforce_pause_resolver_body
    assert "!shouldAllowPostPauseResume" in reinforce_pause_resolver_body
    assert "isPauseGuardActive" in reinforce_pause_resolver_body
    assert 'previousAction == "pause"' in can_resume_body
    assert "now >= localPauseHoldUntil" in can_resume_body
    assert "!shouldRejectResumeAfterPause" in can_resume_body
    assert "!isPauseHoldWindowActive" in can_resume_body
    ...
    job_unsolicited_play_body = _function_body(
        job_now_playing,
        "private var shouldRejectUnsolicitedReaderPlayCommand: Bool",
    )
    assert "ownershipState: musicOwnership.ownershipState" in job_unsolicited_play_body
    ...
    assert "ReaderTransportCommandResolver.shouldRejectUnsolicitedPlayCommand(" in job_unsolicited_play_body
    assert "previousAction: lastReaderTransportAction" in job_unsolicited_play_body
    ...
    assert "now: ProcessInfo.processInfo.systemUptime" in job_unsolicited_play_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in job_unsolicited_play_body
    ...
    assert "guard viewModel.audioCoordinator.isPlaybackRequested else { return }" in job_recovery_schedule_body
    ...
    assert "shouldForceInteractiveReaderTransportResume" in job_interactive_toggle_body
    assert 'forcePlayReaderNowPlayingTransport(source: "interactiveOverride")' in job_interactive_toggle_body
    job_interactive_resume_body = _function_body(
        job_now_playing,
        "private var shouldForceInteractiveReaderTransportResume",
    )
    assert "musicOwnership.isPausedByReaderTransport || musicOwnership.isReaderTransportPauseGuardActive" in job_interactive_resume_body
    assert "musicOwnership.ownershipState == .appleMusicBed" in job_interactive_resume_body
    ...
    assert job_interactive_toggle_body.index("if viewModel.audioCoordinator.isPlaybackRequested ||") < job_interactive_toggle_body.index(
    ...
    assert job_interactive_toggle_body.index('forcePauseReaderNowPlayingTransport(source: "interactiveOverride")') < job_interactive_toggle_body.index(
        "shouldForceInteractiveReaderTransportResume"
    )
    ...
    assert "private var shouldRejectUnsolicitedReaderPlayCommand: Bool" in library_now_playing
    assert "Library reader transport play command ignored unsolicited reader-pause echo" in library_now_playing
    ...
    assert "@State var readerTransportResumeGeneration = 0" in library_playback
    ...
    assert "TVPlayPauseCommandGate.shouldSuppressCurrentPress()" in library_playback
    assert "Library foreground tvOS Play/Pause ignored duplicate physical press" in library_playback
    ...
    assert "Library foreground tvOS Play/Pause ignored reader transport pause echo" not in library_playback
    assert "shouldForceTVReaderNowPlayingResume(ignorePauseHold: true)" in library_playback
    assert 'forcePlayReaderNowPlayingTransport(source: "brokerRequestedResume")' in library_playback
    assert 'forcePlayReaderNowPlayingTransport(source: "foregroundRequestedResume")' in library_playback
    ...
    assert "guard viewModel.audioCoordinator.isPlaybackRequested || viewModel.audioCoordinator.isPlaying else" not in library_adoption_body
    assert library_adoption_body.index("if shouldIgnoreRequestedAppleMusicPauseBeforeReaderAudible") < library_adoption_body.index(
        "if shouldIgnoreStaleAppleMusicPauseAfterReaderPlay"
    )
    ...
    assert "if musicOwnership.ownershipState == .appleMusicBed" in library_mirror_pause_decision_body
    assert "!musicOwnership.isPlaying &&" not in library_mirror_pause_decision_body
    assert "!musicOwnership.isSystemPlaybackPlaying" not in library_mirror_pause_decision_body
    ...
    assert "hasPendingReaderMusicResume" in library_stale_pause_body
    ...
    assert "pendingInteractiveAutoplaySentence != nil" in library_stale_pause_body
    ...
    assert 'reason == "observedNonPlaying" || reason == "deferredObservedNonPlaying"' not in library_honor_adoption_body
    assert 'source == "active observed non-playing"' not in library_honor_adoption_body
    assert 'source == "persistent observed non-playing"' not in library_honor_adoption_body
    ...
    assert "ReaderTransportCommandResolver.shouldAcceptActiveReaderDuplicatePause" in library_accept_body
    assert "ReaderTransportCommandResolver.shouldRejectDuplicateCommand" in library_accept_body
    assert "isReaderPlaybackRequested: viewModel.audioCoordinator.isPlaybackRequested" in library_accept_body
    assert "isReaderPlaying: viewModel.audioCoordinator.isPlaying" in library_accept_body
    ...
    assert "command accepting duplicate pause for active reader" in library_accept_body
    ...
    assert "ReaderTransportCommandResolver.shouldReinforcePauseAfterRejectedPlay(" in library_reinforce_body
    assert "ownershipState: musicOwnership.ownershipState" in library_reinforce_body
    assert "resolvedAction: resolvedAction" in library_reinforce_body
    assert "previousAction: lastReaderTransportAction" in library_reinforce_body
    assert "now: now" in library_reinforce_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in library_reinforce_body
    assert "shouldRejectResumeAfterPause: musicOwnership.shouldRejectReaderTransportResumeAfterPause" in library_reinforce_body
    assert "isPauseHoldWindowActive: musicOwnership.isReaderTransportPauseHoldWindowActive" in library_reinforce_body
    assert "isPauseGuardActive: musicOwnership.isReaderTransportPauseGuardActive" in library_reinforce_body
    ...
    assert "viewModel.audioCoordinator.configureAudioSessionForMixing(\n            true," in library_reassert_play_body
    ...
    assert "ReaderTransportCommandResolver.shouldForceNowPlayingPause(" in library_force_pause_body
    assert "ownershipState: musicOwnership.ownershipState" in library_force_pause_body
    assert "previousAction: lastReaderTransportAction" in library_force_pause_body
    assert "now: ProcessInfo.processInfo.systemUptime" in library_force_pause_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in library_force_pause_body
    assert "shouldRejectResumeAfterPause: musicOwnership.shouldRejectReaderTransportResumeAfterPause" in library_force_pause_body
    assert "isPauseHoldWindowActive: musicOwnership.isReaderTransportPauseHoldWindowActive" in library_force_pause_body
    library_requested_resume_body = _function_body(library_now_playing, "func shouldRecoverTVReaderNowPlayingRequestedPlayback()")
    assert "musicOwnership.ownershipState == .appleMusicBed" in library_requested_resume_body
    assert "viewModel.audioCoordinator.isPlaybackRequested" in library_requested_resume_body
    assert "!viewModel.audioCoordinator.isPlaying" in library_requested_resume_body
    assert "!viewModel.isNarrationAudibleForReaderTransport" in library_requested_resume_body
    assert "musicOwnership.isPlaying" in library_requested_resume_body
    assert "!musicOwnership.isPausedByReaderTransport" in library_requested_resume_body
    assert "!musicOwnership.isReaderTransportPauseGuardActive" in library_requested_resume_body
    assert 'lastReaderTransportAction != "pause"' in library_requested_resume_body
    ...
    assert "ReaderTransportCommandResolver.shouldForceNowPlayingResume(" in library_force_resume_body
    assert "ownershipState: musicOwnership.ownershipState" in library_force_resume_body
    assert "previousAction: lastReaderTransportAction" in library_force_resume_body
    assert "ignorePauseHold: ignorePauseHold" in library_force_resume_body
    assert "now: ProcessInfo.processInfo.systemUptime" in library_force_resume_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in library_force_resume_body
    assert "isReaderPlaybackRequested: viewModel.audioCoordinator.isPlaybackRequested" in library_force_resume_body
    ...
    assert "shouldIgnoreTVReaderTransportBrokerEcho()" not in library_foreground_body
    assert "Library foreground tvOS Play/Pause ignored reader transport pause echo" not in library_foreground_body
    assert library_foreground_body.index("Library foreground tvOS Play/Pause command") < library_foreground_body.index(
        "shouldForceTVReaderNowPlayingPause()"
    )
    ...
    assert "ReaderTransportCommandResolver.shouldIgnoreBrokerEcho(" in library_broker_echo_body
    assert "canForceResume: shouldForceTVReaderNowPlayingResume(ignorePauseHold: true)" in library_broker_echo_body
    ...
    library_unsolicited_play_body = _function_body(
        library_now_playing,
        "private var shouldRejectUnsolicitedReaderPlayCommand: Bool",
    )
    assert "ownershipState: musicOwnership.ownershipState" in library_unsolicited_play_body
    ...
    assert "ReaderTransportCommandResolver.shouldRejectUnsolicitedPlayCommand(" in library_unsolicited_play_body
    assert "previousAction: lastReaderTransportAction" in library_unsolicited_play_body
    ...
    assert "now: ProcessInfo.processInfo.systemUptime" in library_unsolicited_play_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in library_unsolicited_play_body
    ...
    assert "guard viewModel.audioCoordinator.isPlaybackRequested else { return }" in library_recovery_schedule_body
    ...
    assert "shouldForceInteractiveReaderTransportResume" in library_interactive_toggle_body
    assert 'forcePlayReaderNowPlayingTransport(source: "interactiveOverride")' in library_interactive_toggle_body
    library_interactive_resume_body = _function_body(
        library_now_playing,
        "private var shouldForceInteractiveReaderTransportResume",
    )
    assert "musicOwnership.isPausedByReaderTransport || musicOwnership.isReaderTransportPauseGuardActive" in library_interactive_resume_body
    assert "musicOwnership.ownershipState == .appleMusicBed" in library_interactive_resume_body
    ...
    assert library_interactive_toggle_body.index("if viewModel.audioCoordinator.isPlaybackRequested ||") < library_interactive_toggle_body.index(
        'forcePauseReaderNowPlayingTransport(source: "interactiveOverride")'
    )
    ...
    assert "@State var lastReaderTransportPauseToggleTime: TimeInterval = 0" in interactive_view
    ...
    assert "let now = ProcessInfo.processInfo.systemUptime" in playback_toggle_body
    assert "let shouldPauseActiveReader = audioCoordinator.isPlaying || audioCoordinator.isPlaybackRequested" in playback_toggle_body
    assert "shouldSuppressReaderTransportPauseEcho(at: now, shouldPauseActiveReader: shouldPauseActiveReader)" in playback_toggle_body
    assert 'keyboardShortcutDebugLog("[KeyboardShortcut] Interactive ignored duplicate reader transport pause echo")' in playback_toggle_body
    assert "lastReaderTransportPauseToggleTime = now" in playback_toggle_body
    assert playback_toggle_body.index("shouldSuppressReaderTransportPauseEcho(") < playback_toggle_body.index(
        "InteractivePlayerE2EState.recordPlayPauseCommand("
    )
    assert playback_toggle_body.index("lastReaderTransportPauseToggleTime = now") < playback_toggle_body.index(
    ...
    pause_echo_body = _function_body(interactive_input, "private func shouldSuppressReaderTransportPauseEcho(")
    assert "guard !shouldPauseActiveReader else { return false }" in pause_echo_body
    assert "guard playbackToggleOverride != nil else { return false }" in pause_echo_body
    assert "guard shouldCoordinateAppleMusicBedWithReaderTransport else { return false }" in pause_echo_body
    assert "musicCoordinator.isPausedByReaderTransport ||" in pause_echo_body
    assert "musicCoordinator.isReaderTransportPauseGuardActive" in pause_echo_body
    assert "return now - lastReaderTransportPauseToggleTime < 1.0" in pause_echo_body
    ...
    assert "private var lastReaderTransportPauseAdoptionReason: String?" in music
    assert "private var lastReaderTransportPauseAdoptionSource: String?" in music
    assert "var readerTransportPauseAdoptionReason: String? { lastReaderTransportPauseAdoptionReason }" in music
    assert "var readerTransportPauseAdoptionSource: String? { lastReaderTransportPauseAdoptionSource }" in music
    ...
    set_adoption_handler_body = _function_body(music, "func setReaderTransportPauseAdoptionHandler(")
    assert "if isPausedByReaderTransport" in set_adoption_handler_body
    assert "lastReaderTransportPauseAdoptionReason" in set_adoption_handler_body
    assert "lastReaderTransportPauseAdoptionSource" in set_adoption_handler_body
    assert 'source != "reader transport"' in set_adoption_handler_body
    assert "handler(reason, source)" in set_adoption_handler_body
    ...
    assert "lastReaderTransportPauseAdoptionReason = reason" in adopt_pause_body
    assert "lastReaderTransportPauseAdoptionSource = source" in adopt_pause_body
    ...
    assert "#if os(iOS)" in auto_sequence_body
    assert "musicOwnership.simulateReadingBedPlayForE2E()" in auto_sequence_body
    assert "attemptInteractiveStartForE2EIfReady()" in auto_sequence_body
    assert "#else" in auto_sequence_body
    assert "musicOwnership.simulateReadingBedPauseForE2E()" in auto_sequence_body
    assert auto_sequence_body.index("#else") < auto_sequence_body.index(
...
def test_apple_music_reading_bed_keeps_reader_now_playing_controls() -> None:
    ...
    assert "shouldConfirmActiveNarrationNonPlayingAsReaderPause" not in non_playing_body
    assert 'confirmActiveNarrationNonPlayingAsReaderPause(reason: "observedNonPlaying")' not in non_playing_body
    ...
    assert non_playing_body.index("guard shouldTreatObservedNonPlayingAsReaderPause else") < non_playing_body.index(
    ...
    assert "[PlaybackTransport] Apple Music active observed non-playing kept reader active reason=deferredObservedNonPlaying" in deferred_non_playing_body
    ...
    assert deferred_non_playing_body.index('recoverReadingBedForActiveNarration(reason: "deferredObservedNonPlaying")') < deferred_non_playing_body.index(
        "Apple Music deferred non-playing persisted while reader stayed active; keeping narration transport"
    )
    adopt_pause_body = _function_body(music, "private func adoptPauseAsReaderTransport(reason: String, source: String)")
    ...
    assert "hasQueuedMusicForAutoResume" in recovery_body
    assert "canRecoverForE2E" in recovery_body
    ...
    assert "#if os(tvOS)" not in observed_non_playing_body
    assert "ownershipState == .appleMusicBed" not in observed_non_playing_body
    assert "isReaderNarrationActiveForMusicBed" not in observed_non_playing_body
    ...
    assert "!musicOwnership.isSystemPlaybackPlaying" not in job_mirror_body
    assert "guard viewModel.audioCoordinator.isPlaybackRequested || viewModel.audioCoordinator.isPlaying else" in job_mirror_body
    assert "if musicOwnership.ownershipState == .appleMusicBed" in job
    ...
    assert 'clearPendingInteractiveAutoplayForReaderPauseIfNeeded(reason: "jobAudioState")' in job_audio_state_body
    assert job_audio_state_body.index(
        'clearPendingInteractiveAutoplayForReaderPauseIfNeeded(reason: "jobAudioState")'
    ) < job_audio_state_body.index('recoverMutedAppleMusicBedNarrationIfNeeded(reason: "jobAudioState")')
    assert 'recoverPendingInteractiveAutoplayIfNeeded(reason: "jobAudioState")' not in job_audio_state_body
    ...
    assert job_retry_body.index(
        'clearPendingInteractiveAutoplayForReaderPauseIfNeeded(reason: "jobAutoplayRetry")'
    ) < job_retry_body.index("if isInteractiveAutoplaySettled(for: sentence)")
    ...
    assert "!musicOwnership.isPausedByReaderTransport" in job_muted_recovery_body
    ...
    assert 'mirrorAppleMusicPauseToReaderTransport(source: "musicPlayReassert")' in job_watchdog_body
    job_mirror_pause_body = _function_body(job, "private func mirrorAppleMusicPauseToReaderTransport(source: String)")
    assert "readerTransportResumeGeneration &+= 1" in job_mirror_pause_body
    ...
    assert 'refreshReaderNarrationActivityForMusicBed(reason: "jobWatchdog")' in job_watchdog_body
    assert "guard musicOwnership.ownershipState == .appleMusicBed else" in job_watchdog_body
    assert job_watchdog_body.index(
        "guard viewModel.audioCoordinator.isPlaybackRequested || viewModel.audioCoordinator.isPlaying else { return }"
    ) < job_watchdog_body.index("guard musicOwnership.ownershipState == .appleMusicBed else")
    assert job_watchdog_body.index("guard musicOwnership.ownershipState == .appleMusicBed else") < job_watchdog_body.index(
    ...
    job_recover_muted_body = _function_body(job, "private func recoverMutedAppleMusicBedNarrationIfNeeded(reason: String)")
    assert "guard !viewModel.audioCoordinator.isE2ERequestedTransitionPauseActive else { return }" in job_recover_muted_body
    ...
    assert "@State var readerTransportMusicResumeTask: Task<Void, Never>?" in job
    assert "@State var readerTransportResumeGeneration = 0" in job
    assert "readerTransportMusicResumeTask?.cancel()" in job
    assert "private func resolvedReaderTransportAction(forCommand command: String) -> String" in job_now_playing
    job_accept_body = _function_body(job_now_playing, "private func shouldAcceptReaderTransportCommand(_ command: String, resolvedAction: String)")
    assert "shouldBlockReaderTransportResumeAfterPause(resolvedAction: resolvedAction, elapsed: elapsed, now: now)" in job_accept_body
    ...
    assert "static func shouldAcceptActiveReaderDuplicatePause" in transport_resolver
    assert "static var shouldHoldReaderResumeAfterPause: Bool" in transport_resolver
    assert "isMusicPausedByReaderTransport {" in transport_resolver
    ...
    job_play_body = _function_body(job_now_playing, "func playReaderNowPlayingTransport()")
    job_pause_body = _function_body(job_now_playing, "func pauseReaderNowPlayingTransport()")
    job_toggle_interactive_body = _function_body(job_now_playing, "func toggleInteractiveReaderPlaybackTransport()")
    assert job_toggle_interactive_body.index("if viewModel.audioCoordinator.isPlaybackRequested ||") < job_toggle_interactive_body.index(
        "if shouldForceInteractiveReaderTransportResume"
    )
    assert 'forcePauseReaderNowPlayingTransport(source: "interactiveOverride")' in job_toggle_interactive_body
    assert 'forcePlayReaderNowPlayingTransport(source: "interactiveOverride")' in job_toggle_interactive_body
    ...
    assert "ReaderTransportCommandResolver.shouldRejectUnsolicitedPlayCommand(" in job_unsolicited_play_guard_body
    assert "previousAction: lastReaderTransportAction" in job_unsolicited_play_guard_body
    ...
    assert "now: ProcessInfo.processInfo.systemUptime" in job_unsolicited_play_guard_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in job_unsolicited_play_guard_body
    assert 'let resolvedAction = resolvedReaderTransportAction(forCommand: "pause")' in job_pause_body
    ...
    assert "if musicOwnership.isPausedByReaderTransport || musicOwnership.isManuallyPaused" in job_pause_music_body
    assert "musicOwnership.reinforceReadingBedPauseForReaderTransport(reason: lastReaderTransportSource)" in job_pause_music_body
    assert "musicOwnership.pauseReadingBedForReaderTransport()" in job_pause_music_body
    assert job_pause_music_body.index(
        "musicOwnership.reinforceReadingBedPauseForReaderTransport(reason: lastReaderTransportSource)"
    ) < job_pause_music_body.index("musicOwnership.pauseReadingBedForReaderTransport()")
    ...
    assert "guard deferUntilReaderActive else" in job_resume_music_body
    assert "scheduleAppleMusicBedResumeAfterReaderActive()" in job_resume_music_body
    ...
    assert "!musicOwnership.isSystemPlaybackPlaying" not in library_mirror_body
    ...
    assert 'clearPendingInteractiveAutoplayForReaderPauseIfNeeded(reason: "libraryAudioState")' in library_audio_state_body
    assert library_audio_state_body.index(
        'clearPendingInteractiveAutoplayForReaderPauseIfNeeded(reason: "libraryAudioState")'
    ) < library_audio_state_body.index('recoverMutedAppleMusicBedNarrationIfNeeded(reason: "libraryAudioState")')
    assert 'recoverPendingInteractiveAutoplayIfNeeded(reason: "libraryAudioState")' not in library_audio_state_body
    ...
    assert library_retry_body.index(
        'clearPendingInteractiveAutoplayForReaderPauseIfNeeded(reason: "libraryAutoplayRetry")'
    ) < library_retry_body.index("if isInteractiveAutoplaySettled(for: sentence)")
    ...
    assert "!musicOwnership.isPausedByReaderTransport" in library_muted_recovery_body
    ...
    assert 'mirrorAppleMusicPauseToReaderTransport(source: "musicPlayReassert")' in library_watchdog_body
    library_mirror_pause_body = _function_body(library, "private func mirrorAppleMusicPauseToReaderTransport(source: String)")
    assert "readerTransportResumeGeneration &+= 1" in library_mirror_pause_body
    ...
    assert "guard musicOwnership.ownershipState == .appleMusicBed else" in library_watchdog_body
    assert library_watchdog_body.index(
        "guard viewModel.audioCoordinator.isPlaybackRequested || viewModel.audioCoordinator.isPlaying else { return }"
    ) < library_watchdog_body.index("guard musicOwnership.ownershipState == .appleMusicBed else")
    assert library_watchdog_body.index("guard musicOwnership.ownershipState == .appleMusicBed else") < library_watchdog_body.index(
        'musicOwnership.refreshMusicPlaybackSurfaceSuppression(reason: "libraryWatchdog")'
    )
    assert library_watchdog_body.index("if shouldReassertReaderTransportPauseAfterMusicPlay") < library_watchdog_body.index(
        "if shouldMirrorAppleMusicPauseToNarration"
    )
    ...
    library_recover_muted_body = _function_body(library, "private func recoverMutedAppleMusicBedNarrationIfNeeded(reason: String)")
    assert "guard !viewModel.audioCoordinator.isE2ERequestedTransitionPauseActive else { return }" in library_recover_muted_body
    ...
    assert "@State var readerTransportMusicResumeTask: Task<Void, Never>?" in library
    assert "@State var readerTransportResumeGeneration = 0" in library
    assert "readerTransportMusicResumeTask?.cancel()" in library
    assert "private func resolvedReaderTransportAction(forCommand command: String) -> String" in library_now_playing
    library_accept_body = _function_body(library_now_playing, "private func shouldAcceptReaderTransportCommand(_ command: String, resolvedAction: String)")
    assert "shouldBlockReaderTransportResumeAfterPause(resolvedAction: resolvedAction, elapsed: elapsed, now: now)" in library_accept_body
    ...
    library_play_body = _function_body(library_now_playing, "func playReaderNowPlayingTransport()")
    library_pause_body = _function_body(library_now_playing, "func pauseReaderNowPlayingTransport()")
    library_toggle_interactive_body = _function_body(library_now_playing, "func toggleInteractiveReaderPlaybackTransport()")
    assert library_toggle_interactive_body.index("if viewModel.audioCoordinator.isPlaybackRequested ||") < library_toggle_interactive_body.index(
        "if shouldForceInteractiveReaderTransportResume"
    )
    assert 'forcePauseReaderNowPlayingTransport(source: "interactiveOverride")' in library_toggle_interactive_body
    assert 'forcePlayReaderNowPlayingTransport(source: "interactiveOverride")' in library_toggle_interactive_body
    ...
    assert "ReaderTransportCommandResolver.shouldRejectUnsolicitedPlayCommand(" in library_unsolicited_play_guard_body
    assert "previousAction: lastReaderTransportAction" in library_unsolicited_play_guard_body
    ...
    assert "now: ProcessInfo.processInfo.systemUptime" in library_unsolicited_play_guard_body
    assert "localPauseHoldUntil: localReaderTransportPauseHoldUntil" in library_unsolicited_play_guard_body
    assert 'let resolvedAction = resolvedReaderTransportAction(forCommand: "pause")' in library_pause_body
    ...
    assert "if musicOwnership.isPausedByReaderTransport || musicOwnership.isManuallyPaused" in library_pause_music_body
    assert "musicOwnership.reinforceReadingBedPauseForReaderTransport(reason: lastReaderTransportSource)" in library_pause_music_body
    assert "musicOwnership.pauseReadingBedForReaderTransport()" in library_pause_music_body
    assert library_pause_music_body.index(
        "musicOwnership.reinforceReadingBedPauseForReaderTransport(reason: lastReaderTransportSource)"
    ) < library_pause_music_body.index("musicOwnership.pauseReadingBedForReaderTransport()")
    ...
    assert "guard deferUntilReaderActive else" in library_resume_music_body
    assert "scheduleAppleMusicBedResumeAfterReaderActive()" in library_resume_music_body
    ...
    assert 'phase == "observedPause"' in setup_resume_body
    assert 'phase == "observedPauseImmediate"' in setup_resume_body
    ...
    assert "DispatchQueue.main.asyncAfter(deadline: .now() + 18.0)" in auto_sequence_body
    assert "let initialPauseDelay" not in chrome
    ...
    assert "guard !musicOwnership.isPausedByReaderTransport" in late_resume_body
    assert "!musicOwnership.isReaderTransportPauseGuardActive" in late_resume_body
    ...
    assert "guard musicOwnership.ownershipState == .appleMusicBed else" in library_now_playing
    assert "viewModel.audioCoordinator.reassertAudioSession(force: true)" in job_now_playing
    ...
    assert "music_bed_observed_music_pause_kept_reader" in journey
...
def test_apple_music_reader_pause_suppresses_music_surface_until_reader_resumes() -> None:
    ...
    assert "shouldConfirmActiveNarrationNonPlayingAsReaderPause" not in observed_non_playing_body
    assert observed_non_playing_body.index("deferObservedNonPlayingDuringActiveReadingBed") < observed_non_playing_body.index(
        "guard shouldTreatObservedNonPlayingAsReaderPause else"
    )
    assert observed_non_playing_body.index("guard shouldTreatObservedNonPlayingAsReaderPause else") < observed_non_playing_body.index(
        "shouldAdoptObservedNonPlayingImmediately"
    )
    ...
    assert "handleMusicKitReaderTransportPauseAdoption(\n                    reason: musicOwnership.readerTransportPauseAdoptionReason," in job_view
    assert ".onReceive(musicOwnership.$readerTransportPauseAdoptionRevision)" in library_view
    assert "handleMusicKitReaderTransportPauseAdoption(\n                reason: musicOwnership.readerTransportPauseAdoptionReason," in library_view
    for playback_view in (job_view, library_view):
        ...
        assert "handleMusicKitReaderTransportPauseAdoption(reason: reason, source: source)" in playback_view
    job_adoption_body = _function_body(job_view, "private func handleMusicKitReaderTransportPauseAdoption(reason: String? = nil, source: String? = nil)")
    library_adoption_body = _function_body(library_view, "private func handleMusicKitReaderTransportPauseAdoption(reason: String? = nil, source: String? = nil)")
    for adoption_body in (job_adoption_body, library_adoption_body):
        ...
        assert "guard viewModel.audioCoordinator.isPlaybackRequested || viewModel.audioCoordinator.isPlaying else" not in adoption_body
        assert "!shouldHonorAppleMusicPauseAdoptionImmediately(reason: reason, source: source)" in adoption_body
