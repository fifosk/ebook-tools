...
def test_apple_acquisition_api_validates_backend_enum_contracts() -> None:
    ...
    assert "invalidProviderDefaultEligibility(" in api_models_source
    assert "default eligibility for \\(mediaKind) is not discoverable" in api_models_source
    assert "for mediaKind in provider.defaultEligibleMediaKinds where !provider.discoveryMediaKinds.contains(mediaKind)" in api_models_source
...
def test_youtube_dub_acquisition_discovery_is_wired_through_apple_create() -> None:
    ...
    assert "private static func uniqueDownloadStationHints(_ values: [String]) -> [String]" in download_station_source
    assert "var seen = Set<String>()" in download_station_source
