...
- **Apple Interactive Reader:** iPhone/iPad/tvOS playback resolves the same chunk metadata before jump-to-sentence playback; jumps should wait for renderable target-sentence tokens so audio, transcript text, and any sentence image references move together instead of showing the loading state over playing audio. Single-track time seeks should keep an explicit requested sentence-number anchor when present, falling back to the enabled track's gate timing for bare time seeks, so follow-up next/previous navigation and sentence image context do not drift after translation-only slider or bookmark jumps. That temporary single-track anchor must be consumed once live playback reaches the target sentence, otherwise the next translated sentence can keep showing the previous sentence image/text context until the AVPlayer clock settles. Original-only or translation-only mode must also survive chunk/batch setup before default All-track selection runs, and natural end-of-batch advances should reapply the matching audio option plus a sentence-0 next-batch target even while text metadata is still hydrating; otherwise batch image/text context can reset to sequence rendering while single-track audio continues. On tvOS, focused sentence-progress left/right movement should use the same commit path as the touch slider so rendered text, narration, and image context all step one visible sentence together. On iPad, paused lookup keyboard navigation should move the highlighted token through the same rendered sentence rather than shifting bubble controls, so lookup, transcript selection, and sentence image context stay aligned.
- **Apple TV Music bed:** Reader-owned Play/Pause fixes for Apple Music background beds must only change transport state. A remote pause/resume should not advance sentence indexes, clear the rendered sentence, or detach sentence image references while the tvOS Music fullscreen-artwork guard is active. If the system reports Apple Music paused before the reader handler receives a command, Apple playback should adopt the same reader-owned pause guard instead of creating a separate Music-only paused state. After reader resume, Music pause observations are stale only during the short echo window or while narration has not become audible; once the sentence player is audible, that first observed pause must stop narration too without jumping the rendered sentence.
  Reader-owned pause must also cancel pending sequence transitions/audio-ready handoffs and reject stale sentence-player `playing` callbacks, so late AVPlayer or MusicKit callbacks cannot restart narration or move image context after the user asked the combined bed plus sentence track to pause.
  Sequence handoffs should mute and pause the previous AVPlayer item while preserving playback intent before loading or seeking the next Original/Translation item, so stale item tails cannot move audio ahead of transcript or sentence-image context.
