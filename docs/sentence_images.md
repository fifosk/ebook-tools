...
- **Apple TV Music bed:** Reader-owned Play/Pause fixes for Apple Music background beds must only change transport state. A remote pause/resume should not advance sentence indexes, clear the rendered sentence, or detach sentence image references while the tvOS Music fullscreen-artwork guard is active.
