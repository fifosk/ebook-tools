...
- **Apple Interactive Reader:** iPhone/iPad/tvOS playback resolves the same chunk metadata before jump-to-sentence playback; jumps should wait for renderable target-sentence tokens so audio, transcript text, and any sentence image references move together instead of showing the loading state over playing audio. Single-track time seeks should refresh the active visible sentence anchor from the enabled track's gate timing before audio seek, so follow-up next/previous navigation and sentence image context do not drift after translation-only slider or bookmark jumps. On tvOS, focused sentence-progress left/right movement should use the same commit path as the touch slider so rendered text, narration, and image context all step one visible sentence together. On iPad, paused lookup keyboard navigation should move the highlighted token through the same rendered sentence rather than shifting bubble controls, so lookup, transcript selection, and sentence image context stay aligned.
