...
Reading-bed route/schema edits route to `test-backend-reading-beds`,
...
`default_eligible_media_kinds` lists for every advertised provider including
provider extensions, attended Z-Library policy, explicit empty list fields for
that attended-only provider, and token-safe `source_label` values expected by
the Web and Apple discovery pickers. It checks both
...
journey opens Narrate EPUB discovery and verifies the backend-driven provider
picker, query field, and search button are usable; the attended Z-Library
disabled-policy path is verified by the preflight/provider contracts instead of
being exposed as a live discovery picker row. It also selects `Hindi` in the
target-language picker so the full Web-backed language catalog is exercised in
the simulator UI, not only the backend contract. It toggles generated-book
illustrations when needed and
...
with iPhone, iPad, tvOS simulator, and local Mac iPad-style builds.
`make verify-apple-playback-no-regression-candidate` composes that reader
playback sweep with `make verify-apple-music-bed-candidate-dry-run`, giving
playback/UI changes one non-deploying candidate gate before hardware testing.
`make test-changed` selects the no-regression aggregate automatically for reader
playback Swift paths while Music-only files still route to the Music-bed dry-run
and local Apple builds.
...
the current reader session, run `make apple-device-launch-console` with
`APPLE_DEVICE_LAUNCH_PRESERVE_RUNNING=1`, or add `--preserve-running-app` to
the lower-level script, so the helper omits `--terminate-existing` and the
capture does not wipe the playback state before the next remote press.
CoreDevice's raw `--log-output` is kept beside it as
...
backend/Web/Apple contract command groups, simulator smoke profile basics,
physical-device profile basics, iOS/iPadOS capability requirements, and the
explicit "physical deploys only on request" known gates so the golden pipeline
fails before a stale manifest reaches an attended device deploy.
