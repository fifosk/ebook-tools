...
| `make test-backend-acquisition` | `$(PYTHON) -m pytest ...` | Shared-pipeline acquisition provider registry, `/api/acquisition/providers`, runtime descriptor path, token-safe policy notes, and route telemetry slice |
...
| `make test-web-create-intake-focused` | `npm --prefix web test -- --run ...` | Focused Create intake, narration chapter loading, file discovery, voice inventory, narration form, step bar, submit status, and file-dialog Vitest slice |
