...
5. **Persistence** – `modules/services/job_manager/persistence.py` writes `metadata/chunk_XXXX.json` plus a compact `metadata/job.json` (holding the chunk manifest + sentence summaries). Each chunk retains its own top-level `timingTracks`; sentence payloads omit per-word timing tokens and highlighting summaries to avoid duplication. The legacy `metadata/timing_index.json` aggregate is no longer produced for new jobs. `metadata/content_index.json` stores chapter ranges derived from the EPUB so downstream tools can map sentence ranges to chapters. `book_metadata` also captures `translation_provider`/`translation_model` and `transliteration_mode`/`transliteration_model`/`transliteration_module` for translation diagnostics.
