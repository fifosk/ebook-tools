...
5. **Persistence** – `modules/services/job_manager/persistence.py` writes `metadata/chunk_XXXX.json` plus a compact `metadata/job.json` (holding the chunk manifest + sentence summaries). Each chunk retains its own `timingTracks`; the legacy `metadata/timing_index.json` aggregate is no longer produced for new jobs. `metadata/content_index.json` stores chapter ranges derived from the EPUB so downstream tools can map sentence ranges to chapters.
...
Audio segments that carry character timings or word tokens retain those lists via attribute assignments so downstream consumers (timeline builder, separate-track assembly) can reuse them (`modules/render/audio_pipeline.py:936-939`).
...
- Looks up `SentenceAudioMetadata` that the audio worker stored on each translation `AudioSegment`. This metadata describes contiguous translation speech (and any silence) along with low-level `HighlightStep`s derived from character timings or forced alignment (`modules/audio/highlight/core.py:34-220`).
...
- Falls back to sentence-duration proportional events when there is no per-character data. Original audio timing is handled separately via `original_word_tokens` and `timingTracks.original`.
...
These payloads feed both human-readable metadata (e.g., `phase_durations`, `charWeighted` summary) and the later separate-track builder via `SentenceTimingSpec`.
...
- `original_words`, `translation_words`, plus `word_tokens` (translation) and `original_word_tokens` (original).
- Highlight policy flags (`char_weighted_enabled`, `punctuation_boost`, `policy`, `source`) plus the original policy/source from `original_highlighting_summary`.
...
- `SubtitleToolPage` can request colourised subtitle exports via `modules/subtitles/processing.py`, which reads the same timelines and word tokens to generate ASS/SRT with word-level highlighting states.
