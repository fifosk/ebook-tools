...
exists in the current on-device playback state, run the Make shortcut with
`APPLE_DEVICE_LAUNCH_PRESERVE_RUNNING=1` or add `--preserve-running-app` to the
lower-level script so CoreDevice attaches console capture without passing
`--terminate-existing`; this keeps the active reader, Apple Music bed, and
Play/Pause repro state alive for the capture.
...
after the timeout. Each console run also copies the merged public log to a
timestamped sibling archive, with the raw CoreDevice output archived beside it,
so a short follow-up launch cannot erase the richer repro capture. A sleeping
Apple TV foreground launch denial is recoverable:
...
room target, use `--profile cinema`. Both select the `InteractiveReaderTV` scheme,
