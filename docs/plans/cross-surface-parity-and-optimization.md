...
- `web/src/hooks/useLiveMedia.ts` - 191 lines. Status: live media state
  ...
  Vitest lane. Live media event classification for final refreshes, media
  resets, and generated chunk merges now lives in `web/src/hooks/liveMediaEvents.ts`
  with hook-level completion refresh coverage. Initial detail-manifest fallback
  decisions now live in `web/src/hooks/liveMediaLoad.ts`, preserving immediate
  live snapshot rendering while only applying completed-media fallbacks that
  contain visible files. Completion-triggered completed-media refresh now lives
  in `web/src/hooks/liveMediaRefresh.ts`, preserving last-known live snapshots
  when the final media manifest cannot be loaded.
- `web/src/pages/LibraryPage.tsx` - 591 lines. Status: TV/YouTube/library
  ...
  coverage. Library access-policy resolution, selected-item permissions, and
  admin-role detection now live in
  `web/src/pages/library/useLibraryItemPermissions.ts` with focused hook
  coverage for admin, owner, public, private, and grant-backed access. Library
  external focus/search query coordination now lives in
...
- `web/src/components/PlayerPanel.tsx` - 718 lines. Status: selected text
  ...
  focused hook coverage. Autoplay handoff, shell playback/fullscreen callbacks,
  and per-job pending-reader cleanup now live in
  `web/src/components/player-panel/usePlayerPanelLifecycleEffects.ts` with
  focused hook coverage. Viewer renderability, fullscreen preference, document
  and chrome state, and wake-lock activation now live in
  `web/src/components/player-panel/usePlayerPanelViewerState.ts` with focused
  hook coverage. The repo-owned `test-web-playback-focused`
...
- `web/src/components/book-narration/BookNarrationForm.tsx` - 522 lines.
  ...
  orchestration. Base form state now lives in
  `web/src/components/book-narration/useBookNarrationBaseState.ts`, keeping
  shared language defaults, generated-source mode, image defaults, and prefilled
  image flags under focused hook coverage before Apple Create parity checks.
  Ebook/output file selection dialog routing now
...
- In-app changelog: recent entries now follow the dated shard pattern
  (`AppChangelogData+2026-06-30.swift`, `AppChangelogData+2026-07-01.swift`,
...
- Native Create flow. Status: `AppleBookCreateView.swift` now delegates create
  ...
  added. Web Narrate Ebook, Web Video Dubbing, and Apple Create now preserve
  that loaded backend provider response order instead of applying separate
  client-side ranks, while keeping their familiar fallback order before provider
  inventory loads. Backend acquisition
...
- Cross-surface job templates: save a Web configuration and reuse it from Apple. Status:
  ...
  covering encoded subtitle job ids. Backend subtitle job result reads also trim
  padded job identifiers through the shared route-id helper before job-manager
  lookup, keeping Web Subtitle Tool and Apple Create result polling aligned with
  subtitle metadata routes.
...
- Apple Jobs/Library action route contract. Status: Apple jobs list/status,
  ...
  status, SSE event-stream, action, access, metadata refresh/enrich, and book
  metadata lookup routes now share the same route-id cleanup helper used by
  playback state/media/search routes before status, event, pause, resume,
  cancel, delete, restart, access, or metadata service calls.
...
- Apple: release contract, including Markdown/in-app changelog day, visible
  ...
  `make verify-apple-playback-no-regression-candidate` local gate for playback
