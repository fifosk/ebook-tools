...
- **Deployment**: Docker Compose (backend + frontend + PostgreSQL), Synology DSM reverse proxy for TLS
- **Database**: PostgreSQL 16 via SQLAlchemy 2.0 + Alembic (users, sessions, library, config, bookmarks, resume)
- **Storage**: Filesystem-based job storage (Redis optional); media files stay on disk
...
│   ├── database/           # SQLAlchemy 2.0 ORM (engine, base, models/)
...
| Variable | Value | Why |
|---|---|---|
| `DATABASE_URL` | `postgresql://...@postgres/ebook_tools` | Enables PostgreSQL for all 6 storage domains |
...
The backend uses PostgreSQL 16 (via SQLAlchemy 2.0 + Alembic) for persistent
storage of users, sessions, library entries, configuration, bookmarks, and
resume positions. Job metadata and media files remain on the filesystem.

**Dual-mode operation**: When `DATABASE_URL` is set, PG backends are used. When
unset, legacy JSON/SQLite backends are used. The gate is in
`modules/webapi/dependencies.py`.

**Key files**:
- `modules/database/` — Engine singleton, DeclarativeBase, models for 14 tables
- `alembic/` — Schema migrations (auto-run at container startup)
- `scripts/docker-entrypoint.sh` — Runs `alembic upgrade head` before uvicorn
- `scripts/migrate_to_postgres.py` — One-time data migration (`--dry-run`, `--verify`)

**PG repository classes** (match filesystem service interfaces exactly):
- `modules/user_management/pg_user_store.py`, `pg_session_manager.py`
- `modules/services/pg_bookmark_service.py`, `pg_resume_service.py`
- `modules/library/pg_library_repository.py`
- `modules/config_manager/pg_config_repository.py`

**Makefile targets**:
```bash
make db-shell                 # psql into PostgreSQL container
make db-migrate               # run alembic upgrade head
make db-backup                # trigger immediate pg_dump
make db-restore               # restore from gzipped SQL dump
```

**Backup**: The `pg-backup` sidecar runs daily `pg_dump | gzip` with 7-day
retention on NAS (`/Volumes/Backups/ebook-tools/postgres`).
...
- **`modules/database/`** - SQLAlchemy 2.0 ORM (engine, base, models for 14 PG tables)
...
**Backend (runtime)**:
- `DATABASE_URL` - PostgreSQL connection string; enables PG backends for all 6 storage domains
...
- **Grafana**: `https://grafana.langtools.fifosk.synology.me` (Synology DSM reverse proxy → `localhost:3000`)
- **Prometheus**: `https://prometheus.langtools.fifosk.synology.me` (→ `localhost:9090`)
- **Retention**: 90 days or 20GB (Prometheus TSDB)
- **Data**: `/Volumes/Data/Monitoring/{prometheus,grafana}`

### Directory Structure

```
monitoring/
├── prometheus/prometheus.yml              # Scrape config (backend 10s, PG 30s)
├── grafana/
│   ├── provisioning/datasources/          # Auto-provision Prometheus
│   ├── provisioning/dashboards/           # Dashboard provider
│   └── dashboards/{overview,backend,database,metrics-qa}.json
└── postgres-exporter/queries.yaml         # Custom PG queries for app tables
```
...
make monitoring-status   # Health check all 3 services
make docker-status       # Includes monitoring health in output
make db-shell            # psql into PostgreSQL container
```
...
2. Instrument at the call site with a lazy import:
   ...
   def my_function():
       try:
           ...
           pass
   ```
3. For gauge-style metrics that need periodic refresh, add a collector function in `_periodic_gauge_update()`.

### Dashboards

Four auto-provisioned dashboards in the "ebook-tools" folder:
- **Overview** — system up, health, active jobs, request rate, error rate, sessions
- **Backend** — request rate by handler, latency percentiles, pipeline stages, job distribution, auth attempts
- **Database** — PG connections, cache hit ratio, transactions/sec, table sizes, dead tuples
- **Metrics QA** — compact one-panel-per-metric dashboard for visual smoke-testing (10s refresh, 15m window)

### Observability Tests

```bash
pytest -m observability -v   # ~26 tests covering metrics E2E pipeline
make test-observability      # same via Makefile
```

Tests verify: metric presence & type, label cardinality, dashboard PromQL coverage
(every expr references an existing metric), HTTP auto-instrumentation, auth counter
instrumentation, and dashboard JSON structural validity.
...
| `POSTGRES_PASSWORD` | `ebook_tools_dev` | Shared with postgres-exporter |

### First-Time Setup

```bash
# Create persistent storage directories
mkdir -p /Volumes/Data/Monitoring/prometheus /Volumes/Data/Monitoring/grafana
chown -R 472:472 /Volumes/Data/Monitoring/grafana

# Enable pg_stat_statements (already in docker-compose command)
make db-shell
...
- **Deployment (Docker + PostgreSQL)**: `docs/deployment.md`
- **Observability (Prometheus, Grafana, metrics)**: `docs/observability.md`
- **Kubernetes (Helm, k3s, Argo CD)**: `docs/kubernetes.md`
