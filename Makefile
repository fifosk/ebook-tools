...
APPLE_PIPELINE_JOURNEY_PROFILES ?= apple-e2e-journeys iphone ipados tvos iphone-create ipados-create tvos-create ipados-music-bed-sync tvos-music-bed-sync runtime-xcode-readiness ios-uitests-build tvos-uitests-build macos-ipad-style-dry-run macos-ipad-style device-deploy-readiness-dry-run
...
APPLE_DEVICE_IPAD_ID ?= Fifo Ipad Pro
APPLE_DEVICE_IPHONE_ID ?= Fifo iPhone
APPLE_DEVICE_TV_ID ?= Living Room
...
APPLE_PLAYBACK_TRANSPORT_REQUIRED_RELEASE ?= $(APPLE_PLAYBACK_TRANSPORT_CURRENT_RELEASE)
...
test-web-library-focused:
	npm --prefix web test -- --run \
		...
		src/pages/__tests__/LibraryPaginationControls.test.tsx \
...
test-web-job-progress-focused:
	npm --prefix web test -- --run \
		...
		src/components/__tests__/jobProgressActions.test.ts \
		src/components/__tests__/jobProgressGeneratedFiles.test.ts \
		src/components/__tests__/jobProgressImageCluster.test.ts \
...
test-web-playback-focused:
	npm --prefix web test -- --run \
		...
		src/components/__tests__/usePlayerPanelViewerState.test.tsx \
		src/components/__tests__/usePlayerPanelMediaNavigation.test.tsx \
		src/components/__tests__/usePlayerPanelLifecycleEffects.test.tsx \
...
test-web-video-dubbing-focused:
	npm --prefix web test -- --run \
		...
		src/pages/__tests__/useVideoDubbingPageState.test.tsx \
...
apple-device-pull-and-verify-playback-track-reconfigure-log:
	$(MAKE) apple-device-pull-and-verify-playback-transport-log APPLE_PLAYBACK_TRANSPORT_LOG_MODE=track-reconfigure

apple-device-pull-and-verify-current-playback-track-reconfigure-log:
	$(MAKE) apple-device-pull-and-verify-current-playback-transport-log APPLE_PLAYBACK_TRANSPORT_LOG_MODE=track-reconfigure
...
apple-device-verify-playback-track-reconfigure-log:
	$(MAKE) apple-device-verify-playback-transport-log APPLE_PLAYBACK_TRANSPORT_LOG_MODE=track-reconfigure
...
apple-device-verify-current-installed:
	APPLE_DEVICE_REQUIRE_CURRENT_INSTALLED=1 $(MAKE) apple-device-verify-installed APPLE_DEVICE_PROFILE="$(APPLE_DEVICE_PROFILE)" APPLE_DEVICE_ID="$(APPLE_DEVICE_ID)"

apple-device-verify-current-installed-matrix:
	$(MAKE) apple-device-verify-current-installed APPLE_DEVICE_PROFILE=ipad APPLE_DEVICE_ID="$(APPLE_DEVICE_IPAD_ID)"
	$(MAKE) apple-device-verify-current-installed APPLE_DEVICE_PROFILE=iphone APPLE_DEVICE_ID="$(APPLE_DEVICE_IPHONE_ID)"
	$(MAKE) apple-device-verify-current-installed APPLE_DEVICE_PROFILE=appletv APPLE_DEVICE_ID="$(APPLE_DEVICE_TV_ID)"
...
apple-device-deploy-dry-run-matrix:
