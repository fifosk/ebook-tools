...
- **YouTube NAS dubbing:** `modules/services/youtube_dubbing.py` lists downloaded videos under `/Volumes/Data/Video/Youtube` and builds dubbed audio tracks from adjacent ASS subtitles; `/api/subtitles/youtube/dub` now enqueues `youtube_dub` jobs (tracked alongside pipelines/subtitles) via `YoutubeDubbingService`, and the frontend entry `web/src/pages/YoutubeDubPage.tsx` lists NAS videos plus active dubbing jobs.
