...
{"result":{"apps":[{"bundleIdentifier":"com.example.InteractiveReader","name":"InteractiveReader","version":"2026.6.26","bundleVersion":"20260626162"}]}}
JSON
...
assert_contains "${fallback_install_output}" "Verified installed app: InteractiveReader com.example.InteractiveReader version=2026.6.26 build=20260626162" "signed-artifact fallback should still verify installed metadata"
...
assert_contains "${stable_install_output}" "Verified installed app: InteractiveReader com.example.InteractiveReader version=2026.6.26 build=20260626162" "stable signed-artifact install should still verify installed metadata"
...
assert_contains "${stale_install_output}" "Signed artifact build 20260626121 does not match current 20260626162." "stale stable install should fail on current build mismatch"
...
assert_contains "${locked_launch_output}" "Verified installed app: InteractiveReader com.example.InteractiveReader version=2026.6.26 build=20260626162" "locked launch path should still verify installed metadata"
