...
private func runChecks() {
    ...
    requireTrue(manager.isOriginalEnabled, "Toggling the only enabled translation track should restore original")
    requireTrue(manager.isTranslationEnabled, "Toggling the only enabled translation track should keep translation")
    requireEqual(manager.currentMode, .sequence, "Translation-only toggle should restore sequence mode")
    ...
    manager.toggle(.translation, preservingPosition: 6)
    ...
    requireEqual(events[2].1, 6, "Third transition preserved sentence")

    manager.setTracks(original: false, translation: false, preservingPosition: 7)
    ...
    requireEqual(events.count, 4, "Normalization transition count")
    requireEqual(events[3].1, 7, "Normalization should preserve sentence")

    manager.enableSequenceMode(preservingPosition: 8)
    ...
    manager.toggle(kind: .combined, preservingPosition: 10)
    requireEqual(manager.currentMode, .sequence, "Combined kind should restore sequence mode")
    requireEqual(events.last?.1, 10, "Combined kind should preserve sentence")
    manager.toggle(kind: .other, preservingPosition: 11)
    requireEqual(events.last?.1, 10, "Other kind should not change mode")

    requireEqual(manager.resolvePreferredTrackID(for: chunk), "combined", "Sequence preferred audio option")
    manager.setTracks(original: true, translation: false, preservingPosition: 12)
    requireEqual(manager.resolvePreferredTrackID(for: chunk), "original", "Original-only preferred audio option")
    manager.setTracks(original: false, translation: true, preservingPosition: 13)
