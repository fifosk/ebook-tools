...
private func runChecks() {
    ...
    requireTrue(manager.isOriginalEnabled, "Toggling the only enabled translation track should switch to original")
    requireFalse(manager.isTranslationEnabled, "Translation should be disabled after switching to original")
    requireEqual(manager.currentMode, .singleTrack(.original), "Translation-only toggle should enter original-only mode")
    requireEqual(manager.preferredTrack, .original, "Original-only mode should prefer original")
    requireEqual(events.count, 2, "Second mode transition count")
    requireEqual(events[1].0, .singleTrack(.original), "Second transition mode")
    requireEqual(events[1].1, 5, "Second transition preserved sentence")

    manager.setTracks(original: false, translation: false, preservingPosition: 6)
    ...
    requireEqual(events.count, 3, "Normalization transition count")
    requireEqual(events[2].1, 6, "Normalization should preserve sentence")

    manager.enableSequenceMode(preservingPosition: 7)
    requireEqual(events.count, 3, "Enabling already-active sequence mode should not emit a duplicate transition")

    manager.toggle(.translation, preservingPosition: 8)
    requireEqual(manager.currentMode, .singleTrack(.original), "Translation toggle should leave original-only mode")
    manager.toggle(kind: .combined, preservingPosition: 9)
    requireEqual(manager.currentMode, .sequence, "Combined kind should restore sequence mode")
    requireEqual(events.last?.1, 9, "Combined kind should preserve sentence")
    manager.toggle(kind: .other, preservingPosition: 10)
    requireEqual(events.last?.1, 9, "Other kind should not change mode")

    requireEqual(manager.resolvePreferredTrackID(for: chunk), "combined", "Sequence preferred audio option")
    manager.setTracks(original: true, translation: false, preservingPosition: 11)
