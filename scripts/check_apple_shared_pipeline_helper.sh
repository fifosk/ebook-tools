...
music_bed_candidate_dry_run_line="verify-apple-music-bed-candidate-dry-run: test-e2e-ipad-music-bed-sync-dry-run test-e2e-tvos-music-bed-sync-dry-run"
playback_no_regression_candidate_line="verify-apple-playback-no-regression-candidate: verify-apple-reader-playback-candidate verify-apple-music-bed-candidate-dry-run"
create_no_regression_candidate_line="verify-apple-create-no-regression-candidate: test-apple-create-readiness-contract build-apple-ios-simulators build-apple-tvos-simulator build-apple-macos-ipad-style apple-pipeline-ipad-create-readiness-dry-run apple-pipeline-tvos-create-readiness-dry-run"
library_jobs_playback_no_regression_candidate_line="verify-library-jobs-playback-no-regression-candidate: test-backend-library-search-source-isbn test-backend-pipeline-jobs test-backend-playback-state test-backend-playback-media test-backend-offline-export test-web-library-focused test-web-job-progress-focused test-web-playback-focused test-apple-contracts verify-apple-playback-no-regression-candidate"
creation_discovery_no_regression_candidate_line="verify-creation-discovery-no-regression-candidate: test-backend-pipeline-sources test-backend-acquisition test-web-create-intake-focused test-web-video-dubbing-focused verify-apple-create-no-regression-candidate"
living_room_candidate_line="verify-apple-living-room-candidate: verify-apple-dogfood-pipeline verify-apple-music-bed-candidate"
...
assert_contains "${makefile}" "APPLE_PIPELINE_SMOKE_PROFILES ?= ios ipados tvos tvos-cinema" "Makefile should declare shared simulator smoke dry-run profiles"
...
assert_contains "${makefile}" "APPLE_DEVICE_LAUNCH_PRESERVE_RUNNING ?= 0" "Makefile should declare a state-preserving console capture toggle"
...
assert_contains "${makefile}" "${music_bed_candidate_dry_run_line}" "Music-bed candidate dry-run should validate iPad and tvOS Music-bed journey wiring serially"
assert_contains "${makefile}" "${playback_no_regression_candidate_line}" "playback no-regression verification should compose reader playback builds and Music-bed dry-runs"
assert_contains "${makefile}" "${create_no_regression_candidate_line}" "Create no-regression verification should compose Create contracts, local builds, and Create dry-runs"
assert_contains "${makefile}" "${library_jobs_playback_no_regression_candidate_line}" "Library/Jobs playback no-regression verification should compose backend, Web, Apple contracts, and playback gates"
assert_contains "${makefile}" "${creation_discovery_no_regression_candidate_line}" "creation discovery no-regression verification should compose backend, Web, and Apple Create gates"
assert_contains "${makefile}" "${living_room_candidate_line}" "Living Room candidate verification should compose the dogfood pipeline gate and serial Music-bed journeys"
...
assert_contains "${makefile}" "${golden_verify_line}" "golden pipeline verification should fast-forward and source-sync before the non-physical dogfood and Music-bed candidate gates"
...
assert_contains "${makefile}" '$(if $(filter 1 YES yes true TRUE,$(APPLE_DEVICE_LAUNCH_PRESERVE_RUNNING)),--preserve-running-app)' "launch-console helper should expose state-preserving console capture"
...
assert_not_contains "${living_room_candidate_line}" "apple-device-update" "Living Room candidate verification should not depend on physical-device update targets"
assert_not_contains "${living_room_candidate_line}" "run_app_device_deploy.py" "Living Room candidate verification should not route through physical-device deployment"
assert_not_contains "${living_room_candidate_line}" "apple-device-full-entitlement-fallback-install" "Living Room candidate verification should not install signed artifacts"
assert_not_contains "${living_room_candidate_line}" "apple-device-full-entitlement-stable-install" "Living Room candidate verification should not install stable signed artifacts"
assert_not_contains "${playback_no_regression_candidate_line}" "apple-device-update" "playback no-regression verification should not depend on physical-device update targets"
assert_not_contains "${playback_no_regression_candidate_line}" "run_app_device_deploy.py" "playback no-regression verification should not route through physical-device deployment"
assert_not_contains "${playback_no_regression_candidate_line}" "apple-device-full-entitlement-fallback-install" "playback no-regression verification should not install signed artifacts"
assert_not_contains "${playback_no_regression_candidate_line}" "apple-device-full-entitlement-stable-install" "playback no-regression verification should not install stable signed artifacts"
assert_not_contains "${create_no_regression_candidate_line}" "apple-device-update" "Create no-regression verification should not depend on physical-device update targets"
assert_not_contains "${create_no_regression_candidate_line}" "run_app_device_deploy.py" "Create no-regression verification should not route through physical-device deployment"
assert_not_contains "${create_no_regression_candidate_line}" "apple-device-full-entitlement-fallback-install" "Create no-regression verification should not install signed artifacts"
assert_not_contains "${create_no_regression_candidate_line}" "apple-device-full-entitlement-stable-install" "Create no-regression verification should not install stable signed artifacts"
assert_not_contains "${library_jobs_playback_no_regression_candidate_line}" "apple-device-update" "Library/Jobs playback no-regression verification should not depend on physical-device update targets"
assert_not_contains "${library_jobs_playback_no_regression_candidate_line}" "run_app_device_deploy.py" "Library/Jobs playback no-regression verification should not route through physical-device deployment"
assert_not_contains "${library_jobs_playback_no_regression_candidate_line}" "apple-device-full-entitlement-fallback-install" "Library/Jobs playback no-regression verification should not install signed artifacts"
assert_not_contains "${library_jobs_playback_no_regression_candidate_line}" "apple-device-full-entitlement-stable-install" "Library/Jobs playback no-regression verification should not install stable signed artifacts"
assert_not_contains "${creation_discovery_no_regression_candidate_line}" "apple-device-update" "creation discovery no-regression verification should not depend on physical-device update targets"
