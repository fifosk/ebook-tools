...
struct JobPlaybackView: View {
    ...
    @State var lastRecordedSentenceTrack: String?
    ...
    private func handleTVPlayPauseCommand() {
        ...
        guard !TVPlayPauseCommandGate.shouldSuppressCurrentPress() else {
            playbackLogger.info("Job foreground tvOS Play/Pause ignored duplicate physical press")
            return
        }
        #if DEBUG
    ...
    private func handleTVBrokerPlayPauseCommand() {
        ...
        guard !TVPlayPauseCommandGate.shouldSuppressCurrentPress() else {
            playbackLogger.info("Job broker tvOS Play/Pause ignored duplicate physical press")
            return
        }
        #if DEBUG
        e2eTVPlayPauseCommandCount += 1
    ...
    func clearPendingInteractiveAutoplay(reason: String) {
        ...
        playbackLogger.info(
            "Job playback clearing pending interactive autoplay because reader is paused reason=\(reason, privacy: .public)"
        )
        pendingInteractiveAutoplayID = nil
        pendingInteractiveAutoplaySentence = nil
    ...
    private func handleMusicKitReaderTransportPauseAdoption(reason: String? = nil, source: String? = nil) {
        ...
        if shouldKeepReaderActiveDuringAppleMusicPause(reason: reason, source: source) {
            playbackTransportDebugLog(
                "[PlaybackTransport] Job ignored active adopted Apple Music pause while narration requested source=\(source ?? "unknown") reason=\(reason ?? "unknown") requested=\(viewModel.audioCoordinator.isPlaybackRequested) playing=\(viewModel.audioCoordinator.isPlaying) musicPlaying=\(musicOwnership.isPlaying)"
            )
            playbackLogger.info(
        ...
        if shouldIgnoreStaleAppleMusicPauseAfterReaderPlay,
           !shouldHonorAppleMusicPauseAdoptionImmediately(reason: reason, source: source) {
            playbackTransportDebugLog(
                "[PlaybackTransport] Job ignored stale adopted Apple Music pause after reader play source=\(lastReaderTransportSource) musicSource=\(source ?? "unknown")"
            )
            playbackLogger.info(
                "Job playback ignored stale adopted Apple Music pause after reader play source=\(lastReaderTransportSource, privacy: .public) musicSource=\(source ?? "unknown", privacy: .public)"
            )
            resumeAppleMusicBedFromReaderTransportIfNeeded(deferUntilReaderActive: true)
            return
        }
