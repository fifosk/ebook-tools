        ...
        Task { @MainActor in
            for delay in [60_000_000, 180_000_000, 420_000_000] as [UInt64] {
                ...
                guard viewModel.audioCoordinator.isPlaybackRequested ||
                    ...
                    playbackLogger.info(
                        "Library reader transport confirmed pause source=\(source, privacy: .public) requested=false playing=false musicPlaying=\(musicOwnership.isPlaying, privacy: .public)"
                    )
                    publishReaderNowPlayingSnapshot(force: true)
                    return
                }
