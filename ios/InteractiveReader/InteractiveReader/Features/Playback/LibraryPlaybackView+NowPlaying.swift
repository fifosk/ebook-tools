...
extension LibraryPlaybackView {
    ...
    private func performReaderNowPlayingPauseTransport() {
        ...
        clearPendingInteractiveAutoplay(reason: "readerTransportPause")
        viewModel.pauseForReaderTransport()
        pauseAppleMusicBedFromReaderTransportIfNeeded()
    ...
    func confirmReaderTransportPauseAfterCommand(source: String) {
        ...
        Task { @MainActor in
            for delay in [60_000_000, 180_000_000, 420_000_000, 900_000_000, 1_500_000_000] as [UInt64] {
                ...
                let readerStillActive = viewModel.audioCoordinator.isPlaybackRequested ||
                    viewModel.audioCoordinator.isPlaying
                let musicStillActive = musicOwnership.ownershipState == .appleMusicBed &&
                    (musicOwnership.isPlaying || musicOwnership.isSystemPlaybackPlaying)
                guard readerStillActive || musicStillActive else {
                    #if DEBUG
                    ...
                    playbackLogger.info(
                        "Library reader transport confirmed pause source=\(source, privacy: .public) requested=false playing=false musicPlaying=\(musicOwnership.isPlaying, privacy: .public) systemMusicPlaying=\(musicOwnership.isSystemPlaybackPlaying, privacy: .public)"
                    )
                    publishReaderNowPlayingSnapshot(force: true)
                    return
                }
                playbackTransportDebugLog(
                    "[PlaybackTransport] Library confirming reader pause source=\(source) requested=\(viewModel.audioCoordinator.isPlaybackRequested) playing=\(viewModel.audioCoordinator.isPlaying) musicPlaying=\(musicOwnership.isPlaying)"
                )
                playbackLogger.info(
                    "Library reader transport confirming pause source=\(source, privacy: .public) requested=\(viewModel.audioCoordinator.isPlaybackRequested, privacy: .public) playing=\(viewModel.audioCoordinator.isPlaying, privacy: .public) musicPlaying=\(musicOwnership.isPlaying, privacy: .public)"
                )
                if readerStillActive {
                    viewModel.pauseForReaderTransport()
                }
                if musicStillActive {
                    pauseAppleMusicBedFromReaderTransportIfNeeded()
